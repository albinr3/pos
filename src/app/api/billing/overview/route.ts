...
// GET /api/billing/overview - Alias explícito del resumen de facturación
export async function GET(request: NextRequest) {
  try {
    ...
    console.error("Error en GET /api/billing/overview:", error)
    return NextResponse.json(
      { error: error?.message || "Error al obtener resumen de facturación" },
      { status: 500 }
    )
  }
}
